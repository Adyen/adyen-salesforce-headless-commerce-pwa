...
export const getConfig = (opts = {}) => {
    ...
    const explorerSync = cosmiconfigSync(DEPLOY_TARGET, {
        ...
        // Only checks in the `searchFrom` directory to improve performance
        searchStrategy: 'none',
        loaders: {
            '.js': (filepath) => {
 ...
 *
 * @param {string} [configFile] Optional `configFile` that containing the configution. If it
 *      isn't specified, the `configFile` will resolved by resolution rules from the `getConfig`
 * @param {object} opts
 * @param {string} [opts.buildDirectory] Option path to the folder containing the configution.
 * @param {string} [opts.siteId] Optional `siteId` that containing the configution. By default,
 *      it is current site from `app.commerceAPI.parameters.siteId`
 *
 * @returns {object} The configution object
 */
export function getSiteConfig(configFile = '', {siteId = null, ...rest} = {}) {
    if (!siteId) {
    ...
    return getConfig({
        ...rest,
        configFile: `config/${siteId}` + (configFile ? `/${configFile}` : '')
    })
}
