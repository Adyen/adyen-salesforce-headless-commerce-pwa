...
describe('TemporaryBasketService', () => {
    ...
    describe('createTemporaryBasket', () => {
        ...
        it('should create a temporary basket with a product', async () => {
            const product = {
                productId: 'prod123',
                quantity: 2,
                optionItems: [{optionId: 'color', optionValueId: 'red'}]
            }
            const mockResponse = {ok: true, json: jest.fn().mockResolvedValue(mockBasketResponse)}
            temporaryBasketService.apiClient.post.mockResolvedValueOnce(mockResponse)

            const result = await temporaryBasketService.createTemporaryBasket(product)
